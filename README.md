...
| รายการ | ค่า |
| --- | --- |
| Product ID | `sgyjmqpqrw0bzm5a` |
| Model | อ่านได้จาก DP 125 (`model`) |
| Protocol version | **3.5** (family เดียวกับ Smart Gate Module — แนะนำยืนยันด้วย `tinytuya scan`) |
...
(custom function points) ซึ่งไม่ตรงกับ schema มาตรฐานที่ integration รู้จัก
...
> **หมายเหตุ:** ต้องใช้ fork `xZetsubou/hass-localtuya` เพราะอุปกรณ์ family นี้เป็น
...
สแกนหาอุปกรณ์ในวง LAN (จะได้ IP + protocol version — ใช้ยืนยันว่าเป็น 3.5 จริง):
...
| ช่อง | ค่า |
| --- | --- |
| **Name** | ตั้งชื่อได้ตามใจ เช่น `Smart Gate One Key` |
| **Host** | IP ของอุปกรณ์ |
...
| **111** | **Mode** | `mode` | Send and Report | enum | `fullkey`, `onekey` |
...
| 126 | Set Button Open Click | `button_open_click` | Send and Report | enum | `open`, `hide` |
| 127 | Set Button Close Click | `button_close_click` | Send and Report | enum | `close`, `hide` |
...
> **ต่างจาก Smart Gate Module V2:** รุ่นนี้ DP 111 เป็น `mode` (fullkey/onekey)
> แทน `stop_before_close`, ตัด DP 121 (Carlink Stop Before Close) และ
> DP 138 (Toggle Direction) ออก, และ enum ของ DP 126/127 ถูกลดรูป

---

## 7. โหมดควบคุม (Mode)

DP **111 (`mode`)** กำหนดพฤติกรรมของปุ่มควบคุม:

| ค่า | ความหมาย |
| --- | --- |
| `fullkey` | โหมดแยกปุ่ม — ใช้ DP 101/102/103 (เปิด / ปิด / หยุด) แยกกันตามปกติ |
| `onekey` | โหมดปุ่มเดียว — กดปุ่มเดียวสั่งวน (เปิด → หยุด → ปิด → หยุด → ...) |

แนะนำเพิ่มเป็น **select entity** ใน LocalTuya (platform `select`, ผูก DP `111`)
เพื่อสลับโหมดได้จาก Home Assistant

- ถ้าใช้ **fullkey** → map ปุ่มควบคุมตามหัวข้อ 8 ตัวเลือก A/B
- ถ้าใช้ **onekey** → ในทางปฏิบัติมักผูกปุ่มเดียว (เช่น DP 101) เป็น switch/สคริปต์
  ที่กดแล้ววนสถานะ โดยดูผลลัพธ์จริงผ่าน DP 124 (`status`)

---

## 8. map ปุ่มควบคุมประตู (Open / Close / Stop)
...
### ตัวเลือก A — 3 ปุ่มแยกกัน (switch) — เหมาะกับโหมด fullkey
...
> Cover ของ LocalTuya ออกแบบมาสำหรับ **command DP ตัวเดียว** แต่ประตูนี้แยกเป็น
> 3 DP หากต้องการใช้ cover เต็มรูปแบบ แนะนำ (ฝั่ง firmware) เพิ่ม DP command
> แบบ enum ตัวเดียว เช่น `control: open / close / stop`
...
| Mode | select | `111` | สลับ fullkey / onekey |
...
## 9. ทดสอบด้วย tinytuya (นอก Home Assistant)
...
d.set_version(3.5)              # family นี้เป็น 3.5 (ยืนยันด้วย scan)
...
# print("mode:",  d.set_value(111, "onekey"))  # สลับโหมด
```
...
| กดปุ่มไม่ทำงานตามคาด | อยู่ผิดโหมด (fullkey/onekey) | เช็ก DP 111 ให้ตรงกับวิธี map ปุ่ม |
